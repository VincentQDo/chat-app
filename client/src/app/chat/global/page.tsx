...
export default function GlobalChat() {
  ...
  const [rooms, setRooms] = useState([
    { id: "global", name: "Global Chat", type: "room" },
  ]);
  ...
  const getCurrentRoomTypingUsers = () => {
    return typingUsers
      .filter(
        (user) => user.roomId === selectedRoom && user.userId !== userName
      )
  ...
  const markUnreadAsRead = () => {
    const unreadMessages = messages.filter(
      (msg) =>
        msg.userId !== userName &&
        !msg.statuses?.some(
          (status) => status.userId === userName && status.status === "read"
        )
    );
  ...
  useEffect(() => {
    ...
    socket.current.on("delivered", (data: { success: { statuses: any[] } }) => {
      ...
      setMessages((prevMessages) =>
        prevMessages.map((msg) => {
          const updatedMessageStatuses = updatedStatuses.filter(
            (status: any) => status.messageId === msg.messageId
          );
      ...
      (data: { userId: string; roomId: string }) => {
        setTypingUsers((prev) => {
          const exists = prev.find(
            (user) => user.userId === data.userId && user.roomId === data.roomId
          );
    ...
    socket.current.on(
      ...
      (data: { userId: string; roomId: string }) => {
        setTypingUsers((prev) =>
          prev.filter(
            (user) =>
              !(user.userId === data.userId && user.roomId === data.roomId)
          )
        );
      }
    );
    ...
    socket.current.on("disconnect", (reason) => {
      console.error(
        "Disconnected from server for the following reason: ",
        reason
      );
      setMessages((prevMessages) => [
        {
  ...
  return (
    <>
      ...
      <div
        className={cn("flex-1 overflow-y-auto p-4 min-h-0", {
          "space-y-4": !isCompact,
        })}
        ref={messagesContainerRef}
        onScroll={handleMessageScrolling}
      >
        {isCompact
          ? messages.map((msg, index) => (
              <AppMessage key={index} message={msg} />
            ))
          : messages.map((msg, index) => (
              <AppMessageLarge key={index} message={msg} />
            ))}
